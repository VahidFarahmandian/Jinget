...
//set picker on input/button
window.initializeDatePicker = (dotnet, params = {}) => {
    params = $.extend({}, datePickerDefaults, params);
    ...
    let settings = {
        ...
        onDayClick: (a) => {
            dotnet.invokeMethodAsync('OnJSDatePickerSelectedDateChanged', a.selectedDate.toLocaleString());
        }
    };
    
    new jinget_dp.JingetDateTimePicker(element, settings);

    //new JingetDateTimePicker(element, settings);
    ...
    //onrmalize an array of dates toan array of js dates
